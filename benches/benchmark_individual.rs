...
impl EntropyImpl {
	...
	fn entropy(self, bit_data: BitDataSet) -> Result<(BitDataSet, Vec<(usize, f64)>), EntroGdError> {
		match self {
...
impl DecompressRowsImpl {
	...
	fn process(
		self,
		input: (Arc<CompressedData>, Vec<usize>),
	...
	for case in prepared_cases {
		...
		for implementation in implementations.iter().copied() {
			group.bench_function(
				BenchmarkId::from_parameter(format!("{}/{}", impl_label(implementation), case.name)),
				|b| {
					b.iter_batched(
						|| make_input(case),
						|input| {
...
fn benchmark_filter_steps(c: &mut Criterion) {
	...
	let prepared_cases: Vec<PreparedCase> = step_bench_cases()
		.into_iter()
		.map(prepare_case)
		.collect();
	...
	bench_step_group(
		...
		|case| (case.rows_input_seed.0.clone(), case.rows_input_seed.1.clone()),
		|implementation, input, _case| implementation.process(input),
	);
}
