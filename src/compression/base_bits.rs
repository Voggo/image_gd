...
impl BaseBitBatchGroups {
    ...
    pub fn add_bit_positions(&mut self, bit_data: &BitDataSet, bit_positions: &[usize]) -> usize {
        ...
        // Keep only unique, not-yet-selected positions while preserving order.
        let mut new_bit_positions = Vec::with_capacity(bit_positions.len());
        for &bit_position in bit_positions {
            if self.base_bit_mask[bit_position]
                || new_bit_positions
                    .iter()
                    .any(|&existing| existing == bit_position)
            {
                continue;
            }
        ...
        if new_bit_positions.len() >= usize::BITS as usize || new_bit_positions.len() > 16 {
        ...
        self.base_bit_positions.extend_from_slice(&new_bit_positions);
        self.num_bits_per_base += new_bit_positions.len();
